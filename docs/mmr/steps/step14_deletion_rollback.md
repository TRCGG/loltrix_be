...
total_games -= 1
total_wins  -= w
total_mmr  = Σ(pos_games)=0 ? INITIAL_MMR(1300)
                            : round( Σ(pos_mmr × pos_games) / Σ(pos_games) )   -- games>0 포지션만
```

> ⚠️ **분모 0 가드**: 유저의 마지막(유일) 경기를 삭제하면 모든 `pos_games=0`이 되어 `Σ(pos_games)=0` → 나눗셈 0. 이때 `total_mmr`은 초기값(1300)으로 둔다. (step09 `buildSummaryRow`의 가중평균도 동일 가드)
...
| soft delete | metric/queue/result/history `is_deleted=true`. **영구 보존**(hard delete 없음, [step12](./step12_crons.md)) |
| 복구 | 없음 (조회는 `is_deleted=false`로 제외, 데이터는 audit용 보존) |
...
- **연관**: [step11 §4](./step11_admin_api.md)(경기 제외) · [step12](./step12_crons.md)(파티션·알람)
</content>
