...
삭제 발생
  ...
  └─ done 경기 (MMR 반영됨) → 즉시 역산 롤백 + soft delete:
        ...
        3. 이 경기의 mmr_participant_metric / mmr_match_queue / mmr_match_result / mmr_history
           → is_deleted = true (soft delete, 복구 없음 → cleanup이 hard delete)
        ※ 전체 RECALC 없음
```
...
> **soft delete**: 경기·MMR row는 모두 `is_deleted=true`로 표시(복구 없음). 30일 후 cleanup이 hard delete.
...
incremental·RECALC·cleanup **모든 비동기 작업을 `mmr_job` 큐 하나로 직렬화**한다. 별도 advisory lock이나 `mmr_guild_state` 행 잠금(NOWAIT)을 쓰지 않는다. (BASELINE은 관리자 수동·<60초라 큐에 넣지 않고 동기 admin API로 처리.)
...
- 누적 순서가 무의미한 잡(`CLEANUP`, `guild_id` 없음)은 이 제약을 받지 않는다.
...
**재구독**: 해지 후 경과 기간과 무관하게 **신규와 동일한 전체 RECALC**(다음 10시 예약). 해지 때 정리된 데이터를 RECALC가 metric에서 재구축한다([step07](./steps/step07_subscription.md)). 구독 해지 유예는 없음(metric 전 길드 보존이라 불필요).
...
DELETE /replays/:id   (soft delete)
  ...
  └─ mmr_match_queue.status 분기:
       ...
       └─ done → 즉시 역산 롤백 (§2-C):
                  ...
                  (전체 RECALC 없음, 복구 없음 → 30일 후 cleanup이 hard delete)
```
...
```
1. soft delete 30일 경과분 hard delete (CLEANUP)
   (mmr_match_result / mmr_history / mmr_member_summary / mmr_match_queue)

2. monthly partition 다음달 분 미리 생성 (mmr_history)

3. 24시간 이상 wait인 custom_match 알람
```

> daily cron 자체는 cleanup·파티션·알람만 한다(삭제는 역산 롤백이라 RECALC 트리거 없음). 다만 **(재)구독 RECALC가 `scheduled_date=10시`로 예약돼 있어, 같은 10시 창에 worker가 그 RECALC들도 함께 픽업**한다 → 10시 = "무거운 배치 창"(RECALC + cleanup).
...
| 항목 | 결정 |
|---|---|
| mmr_job 테이블 | **유지.** incremental·RECALC·cleanup이 큐 경유(§2-E). job_type 3종: `INCREMENTAL_BATCH`/`RECALC`/`CLEANUP`. BASELINE은 동기 admin API |
