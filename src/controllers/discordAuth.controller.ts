...
const discordGuildAccessService = new DiscordGuildAccessService();
...
export const getGmokGuilds = async (
  ...
  try {
    ...
    // 2. 활성 권한 조회 + gmok 길드 목록 조회 (오케스트레이션은 서비스로 이관)
    const guildsData = await discordGuildAccessService.getGmokGuildsForMember(
      discordMemberId,
      accessToken,
    );
