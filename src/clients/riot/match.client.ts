 ...
 */
export async function getMatch(matchId: string, caller: string): Promise<MatchV5Dto> {
  const cfg = getRiotConfig();
  return riotRequest<MatchV5Dto>(
    `${cfg.matchBaseUrl}/matches/${encodeURIComponent(matchId)}`,
    cfg.apiKey,
    { route: '/matches/{matchId}', caller },
  );
}
 ...
 */
export async function getMatchTimeline(matchId: string, caller: string): Promise<MatchTimelineDto> {
  const cfg = getRiotConfig();
  return riotRequest<MatchTimelineDto>(
    `${cfg.matchBaseUrl}/matches/${encodeURIComponent(matchId)}/timeline`,
    cfg.apiKey,
    { route: '/matches/{matchId}/timeline', caller },
  );
}
