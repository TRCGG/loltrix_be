...
const applicationBody = {
  ...
  subPositions: z
    .array(z.enum(COMPETITION_SUB_POSITIONS))
    .max(COMPETITION_POSITIONS.length - 1)
    .optional(),
  champions: z
...
router.get(
  ...
  /* #swagger.auto = false
    ...
    #swagger.description = '로그인한 사용자가 이 대회에 넣은 신청 한 건을 반환합니다(riotName·riotNameTag, champions는 id·한글명·영문명·riotKey(Data Dragon 이미지 조회용 라이엇 챔피언 key, 미상이면 null)로 채워집니다). appliedByDisplayName은 신청자의 디스코드 표시명으로, 이 길드의 별명 → 전역 표시명 → member id 순으로 정해집니다(예: 폿신). 신청이 없으면 404(application-not-found), 봇 요청은 403.'
    #swagger.security = [{ "session": [] }]
    #swagger.parameters['guildId'] = { in: 'path', description: '길드 ID (Base64)', required: true, type: 'string' }
    #swagger.parameters['competitionId'] = { in: 'path', required: true, type: 'integer' }
  */
